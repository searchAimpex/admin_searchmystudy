...
const CreateLoanLead = ({ ele, handleClose, fetchData }) => {
    ...
    // align keys with your Mongoose/schema — keep fields the user asked for
    const initial = {
    ...
    const [uploads, setUploads] = useState({
        // keep previews for template, offerLetter and passportDoc
        template: { progress: 0, preview: initial.template || null, loading: false },
        offerLetter: { progress: 0, preview: initial.offerLetter || null, loading: false },
        passportDoc: { progress: 0, preview: initial.passportDoc || null, loading: false },
    });
                            ...
                            <div className="modal-body" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
                                <div className="row g-3">
                                    ...
                                    <div className="col-md-4">
                                        ...
                                        <input name="emailID" type="email" value={formValues.emailID || ""} onChange={handleInputChange} className="form-control" />
                                    </div>

                                    <div className="col-md-4">
                                        <label className="form-label">DOB</label>
                                        <input name="dob" type="date" value={formValues.dob || ""} onChange={handleInputChange} className="form-control" />
                                    </div>

                                    <div className="col-md-4">
                                        <label className="form-label">Amount</label>
                                        <input name="amount" type="number" value={formValues.amount || ""} onChange={handleInputChange} className="form-control" />
                                    </div>
                                    ...
                                    <div className="col-md-6">
                                        ...
                                        <input type="file" name="offerLetter" id="offerLetter" accept="image/*,application/pdf" onChange={(e) => handleFileChange(e, 'offerLetter')} className="form-control" />
                                        {uploads.offerLetter?.preview && (
                                            <div className="mt-2">
                                                {typeof uploads.offerLetter.preview === 'string' && uploads.offerLetter.preview.toLowerCase().includes('.pdf') ? (
                                                    <a href={uploads.offerLetter.preview} target="_blank" rel="noreferrer">Open Offer Letter (PDF)</a>
                                                ) : (
                                                    <img src={uploads.offerLetter.preview} alt="offerLetter" style={{ maxWidth: 200, maxHeight: 120 }} />
                                                )}
                                                <div>Progress: {Math.round(uploads.offerLetter.progress)}%</div>
                                            </div>
                                        )}
                                    </div>

                                    <div className="col-md-6">
                                        <label htmlFor="passportDoc" className="form-label">Passport Document</label>
                                        <input type="file" name="passportDoc" id="passportDoc" accept="image/*,application/pdf" onChange={(e) => handleFileChange(e, 'passportDoc')} className="form-control" />
                                        {uploads.passportDoc?.preview && (
                                            <div className="mt-2">
                                                {typeof uploads.passportDoc.preview === 'string' && uploads.passportDoc.preview.toLowerCase().includes('.pdf') ? (
                                                    <a href={uploads.passportDoc.preview} target="_blank" rel="noreferrer">Open Passport Doc (PDF)</a>
                                                ) : (
                                                    <img src={uploads.passportDoc.preview} alt="passportDoc" style={{ maxWidth: 200, maxHeight: 120 }} />
                                                )}
                                                <div>Progress: {Math.round(uploads.passportDoc.progress)}%</div>
                                            </div>
                                        )}
                                    </div>
                                </div>
